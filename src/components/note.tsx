...
import { InfoIcon, AlertTriangleIcon, AlertCircle, CheckCircle2, BookmarkIcon } from "lucide-react"
...
export function Note({ children, type = "none", className, hideIcon = false }: NoteProps) {
  ...
  const icons = {
    ...
    tip: <BookmarkIcon className="h-5 w-5 text-purple-500 dark:text-purple-400" />,
  }
  ...
  const typeStyles = {
    ...
    alert: "border-red-100 bg-red-50 text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300",
    success:
  ...
  return (
    <div className={cn(baseStyles, typeStyles[type], className)}>
      ...
      <div className={cn("absolute inset-0 bg-gradient-to-r pointer-events-none", gradientStyles[type])} />
